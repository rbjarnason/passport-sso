...
class AudkenniStrategy extends OpenIDConnectStrategy {
  constructor(options, verify) {
    ...
    console.log(
      `AudkenniStrategy initialized with PKCE method: ${this._pkceMethod}`
    );
  }
  ...
  authenticate(req, options) {
    ...
    if (this._options.acrValues) {
      options.acr_values = this._options.acrValues;
    }

    // Add RELATEDPARTY to scope if needed
    ...
    if (!req.query || !req.query.code) {
      ...
      if (req.session && req.session.pkceCodeVerifier) {
        ...
        console.error("AudkenniStrategy: Missing pkceCodeVerifier in session");
        return this.error(new Error("Missing PKCE code verifier"));
      }
    }
  ...
  authorizationParams(options) {
    ...
    if (this._pkceMethod) {
      ...
      options.req.session.pkceCodeVerifier = verifier;
      console.log(
      ...
      switch (passportOpts.provider) {
        ...
        case "audkenni":
          const strategyOptions = {
            ...
            acrValues: passportOpts.options.acrValues || "sim",
            relatedParty: passportOpts.options.clientID, // or another identifier if needed
