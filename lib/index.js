module.exports = function () {
  ...
  PassportSSO.loadStrategies = function (providers, authorize, login) {
    ...
    for (var i = 0; i < providers.length; i++) {
      ...
      console.log(`1-------------------> ${JSON.stringify(passportOpts, null, 2)}`);
      ...
      switch (passportOpts.provider) {
        ...
        case "oidc":
        case "audkenni":
          passportOpts.options = providerOpts.options;
          Providers[passportOpts.name] = providerOpts;

          const StrategyClass = require(providerOpts.strategyPackage)[providerOpts.strategyObject];

          console.log("OIDC Strategy options:", JSON.stringify(passportOpts.options, null, 2));
          console.log("StrategyClass:", StrategyClass);
          ...
          Passport.use(
            ...
            new StrategyClass(
              ...
              function(issuer, profile, done) {
                console.log("Verify callback called with issuer:", issuer);
                console.log("Profile:", JSON.stringify(profile, null, 2));
                done(null, profile);
              }
            )
          );
          console.log("OIDC Strategy created successfully");
          continue;
        default:
          Providers[passportOpts.name] = providerOpts;
  ...
  PassportSSO.getPassportOidcOptions = function (providerOpts) {
    console.log(`2-------------------> ${JSON.stringify(providerOpts, null, 2)}`);
    var passportOpts = {
      provider: providerOpts.provider,
      name: providerOpts.name,
      protocol: providerOpts.protocol,
