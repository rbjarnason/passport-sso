...
class AudkenniStrategy extends OpenIDConnectStrategy {
  ...
  authenticate(req, options) {
    ...
    // Debugging: Log entire req object to inspect it
    //console.log("AudkenniStrategy.authenticate - Full req object:", JSON.stringify(req, null, 2));

    if (!req || !req.headers) {
      console.error("AudkenniStrategy.authenticate - WARNING: req or req.headers is not defined!");
    } else {
      console.log("AudkenniStrategy.authenticate - req.headers:", JSON.stringify(req.headers, null, 2));
    }
    ...
    if (!req.query || !req.query.code) {
      ...
      const codeVerifier = this.getCookie(req, "pkceCodeVerifier");
      console.log(`AudkenniStrategy: Retrieved codeVerifier from cookie: ${codeVerifier}`);
      if (codeVerifier) {
        options.code_verifier = codeVerifier;
        this.clearCookie(req, "pkceCodeVerifier");
        console.log(
          "AudkenniStrategy: Code verifier retrieved and cookie cleared:",
          options.code_verifier
        );
      } else {
        console.warn("AudkenniStrategy: Missing pkceCodeVerifier in cookie");
      }
    }
  ...
  authorizationParams(req, options) {
    ...
    if (this._pkceMethod) {
      ...
      this.setCookie(req, "pkceCodeVerifier", verifier);
      console.log(`AudkenniStrategy: PKCE verifier stored in cookie.`);
    }
  ...
  tokenParams(options) {
    ...
    console.log(`AudkenniStrategy: Initial token params:`, params);

    if (options.code_verifier) {
      params.code_verifier = options.code_verifier;
      console.log(
        "AudkenniStrategy: Code verifier added to token params:",
  ...
  setCookie(req, name, value) {
    console.log(`AudkenniStrategy: Setting cookie: ${name}=${value}`);
    const cookie = `${name}=${value}; Path=/; HttpOnly; Secure`;

    // Ensure req.headers and req.headers.cookie exist
    if (!req.headers) {
      req.headers = {};
    }
    ...
    console.log(`AudkenniStrategy: Cookie set: ${req.headers.cookie}`);
  }

  getCookie(req, name) {
    console.log(`AudkenniStrategy: Getting cookie: ${name}`);

    // Ensure req.headers.cookie exists
    if (!req.headers || !req.headers.cookie) {
      console.log(`AudkenniStrategy: No cookies found.`);
      return null;
    }
    ...
    console.log(`AudkenniStrategy: Retrieved cookie value: ${cookieValue}`);
    return cookieValue;
  }

  clearCookie(req, name) {
    console.log(`AudkenniStrategy: Clearing cookie: ${name}`);
    const expiredCookie = `${name}=; Path=/; HttpOnly; Secure; Expires=Thu, 01 Jan 1970 00:00:00 GMT`;
    ...
    if (!req.headers.cookie) {
      req.headers.cookie = expiredCookie;
    } else {
      req.headers.cookie += `; ${expiredCookie}`;
    }

    console.log(`AudkenniStrategy: Cookie cleared: ${req.headers.cookie}`);
  }
}
...
module.exports = function () {
    ...
    for (var i = 0; i < providers.length; i++) {
      ...
      switch (passportOpts.provider) {
        ...
        case "audkenni":
          ...
          console.log(
            `3-------------------> ${JSON.stringify(strategyOptions, null, 2)}`
          );
  ...
  PassportSSO.authenticate = function (strategy, options, req, res, cb) {
    console.log(
      `PassportSSO.authenticate  -------------------> ${JSON.stringify(
        options,
        null,
        2
      )} strategy ${strategy}`
  ...
  PassportSSO.callback = function (strategy, token, options, req, res, cb) {
    console.log(`PassportSSO.callback  -------------------> ${strategy}`);
  ...
  PassportSSO.getPassportOidcOptions = function (providerOpts) {
    console.log(
      `getPassportOidcOptions 2-------------------> ${JSON.stringify(
        providerOpts,
        null,
        2
      )}`
    ...
    console.log(
      `getPassportOidcOptions 3-------------------> ${JSON.stringify(
        passportOpts,
        null,
        2
      )}`
