...
class AudkenniStrategy extends OpenIDConnectStrategy {
  ...
  async authenticate(req, options) {
    ...
    if (!req || !req.redisClient) {
      console.error("AudkenniStrategy.authenticate - WARNING: req or req.redisClient is not defined!");
    ...
    if (!req.query || !req.query.code) {
      ...
      const params = await this.authorizationParams(req, options);
      options = { ...options, ...params };
    } else {
      // Token phase
      console.log("AudkenniStrategy: Entering token phase");
      const codeVerifier = await this.getVerifierFromRedis(req, "pkceCodeVerifier");
      console.log(`AudkenniStrategy: Retrieved codeVerifier from Redis: ${codeVerifier}`);
      if (codeVerifier) {
        options.code_verifier = codeVerifier;
        await this.clearVerifierFromRedis(req, "pkceCodeVerifier");
        console.log(
  ...
  async authorizationParams(req, options) {
    ...
    if (this._pkceMethod) {
      ...
      // Store the verifier in Redis
      try {
        await this.storeVerifierInRedis(req, "pkceCodeVerifier", verifier);
      } catch (err) {
        console.error(`AudkenniStrategy: Error storing PKCE verifier in Redis: ${err}`);
        return this.error(new Error("Error storing PKCE verifier in Redis"));
      }
      console.log(`AudkenniStrategy: PKCE verifier stored in Redis.`);
    }
  ...
  async storeVerifierInRedis(req, key, value) {
    ...
    try {
      await req.redisClient.set(redisKey, value, { EX: 600 }); // EX: 600 sets an expiration time of 10 minutes
    } catch (err) {
      console.error(`AudkenniStrategy: Error storing verifier in Redis: ${err}`);
      return this.error(new Error("Error storing verifier in Redis"));
    }

    console.log(`AudkenniStrategy: Stored verifier in Redis with key: ${redisKey}`);
  }
  ...
  async getVerifierFromRedis(req, key) {
    ...
    try {
      ...
      console.log(`AudkenniStrategy: Retrieved verifier from Redis with key: ${redisKey}`);
      return value;
    } catch (err) {
      console.error(`AudkenniStrategy: Error retrieving verifier from Redis: ${err}`);
      return null;
    }
  }
  ...
  async clearVerifierFromRedis(req, key) {
    ...
    try {
      ...
      console.log(`AudkenniStrategy: Cleared verifier in Redis with key: ${redisKey}`);
    } catch (err) {
      console.error(`AudkenniStrategy: Error clearing verifier in Redis: ${err}`);
    }
  }
