...
class AudkenniStrategy extends OpenIDConnectStrategy {
  ...
  authenticate(req, res, options) {
    ...
    if (!req.query || !req.query.code) {
      ...
      const params = this.authorizationParams(req, res, options);
      ...
      if (codeVerifier) {
        ...
        this.clearCookie(res, "pkceCodeVerifier");
        console.log(
  ...
  authorizationParams(req, res, options) {
    ...
    if (this._pkceMethod) {
      ...
      this.setCookie(res, "pkceCodeVerifier", verifier);
      console.log(
        `AudkenniStrategy: PKCE verifier stored in cookie.`
      );
    }
  ...
  setCookie(res, name, value) {
    ...
    console.log(`AudkenniStrategy: Cookie set: ${cookie}`);
  }
  ...
  clearCookie(res, name) {
    ...
    const cookie = `${name}=; Path=/; HttpOnly; Secure; Expires=Thu, 01 Jan 1970 00:00:00 GMT`;
    const existingCookies = res.getHeader('Set-Cookie');
    ...
    console.log(`AudkenniStrategy: Cookie cleared: ${cookie}`);
  }
}
