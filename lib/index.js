...
class AudkenniStrategy extends OpenIDConnectStrategy {
  ...
  async authenticate(req, options) {
    this._req = req; // Store the req object
    options = options || {};
    ...
    if (!req.query || !req.query.code) {
      ...
      const params = await this.authorizationParams(options);
  ...
  async authorizationParams(options) {
    const params = await super.authorizationParams(options);
    console.log(
    ...
    if (this._pkceMethod) {
      ...
      try {
        await this.storeVerifierInRedis(this._req, "pkceCodeVerifier", verifier);
      } catch (err) {
