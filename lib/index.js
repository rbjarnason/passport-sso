...
class AudkenniStrategy extends OpenIDConnectStrategy {
  ...
  proceedWithAuthentication(req, options) {
    ...
    // No global monkey-patching – code_verifier will be added in tokenParams()
  ...
  // Inject the PKCE code_verifier into the token request body that is sent
  // during the authorization-code exchange.  This method is called by
  // passport-openidconnect just before it exchanges the code.
  tokenParams(options) {
    // Preserve the parameters that the base strategy sets up (scope, grant_type, …).
    const params = (typeof super.tokenParams === "function") ? super.tokenParams(options) : {};
