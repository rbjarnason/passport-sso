...
class AudkenniStrategy extends OpenIDConnectStrategy {
  ...
  authenticate(req, options, extra) {
    ...
    if (!req.query || !req.query.code) {
      ...
      this.getVerifierFromRedis(req, `pkceSet`)
        .then((candidates) => {
          console.log(
            `AudkenniStrategy: Retrieved PKCE verifier candidates from Redis: ${JSON.stringify(candidates)}`
          );

          if (candidates && candidates.length) {
            options.code_verifier_candidates = candidates;
          } else {
            console.warn("AudkenniStrategy: No PKCE verifiers found in Redis set");
          }
  ...
  proceedWithAuthentication(req, options) {
    ...
    // Override the OAuth2 getOAuthAccessToken method to iterate over stored PKCE verifiers
    const originalGetOAuthAccessToken = this._oauth2.getOAuthAccessToken;
    this._oauth2.getOAuthAccessToken = (code, params, callback) => {
      console.log(`AudkenniStrategy: getOAuthAccessToken called with code: ${code} and params: ${JSON.stringify(params, null, 2)}`);
      const self = this;
      const candidates = (options.code_verifier_candidates || []).slice();
      console.log(`AudkenniStrategy: candidates: ${JSON.stringify(candidates, null, 2)}`);
      const attempt = () => {
        if (candidates.length === 0) {
          // Fallback to any single verifier provided directly (or none)
          if (options.code_verifier) {
            params.code_verifier = options.code_verifier;
          }
          console.log(`AudkenniStrategy: no candidates, using single verifier`);
          return originalGetOAuthAccessToken.call(self._oauth2, code, params, callback);
        }

        params.code_verifier = candidates.shift();
        originalGetOAuthAccessToken.call(self._oauth2, code, params, (err, accessToken, refreshToken, results) => {
          console.log(`AudkenniStrategy: originalGetOAuthAccessToken.call(self._oauth2, code, params, (err, accessToken, refreshToken, results)`,
            JSON.stringify(err, null, 2),
            JSON.stringify(accessToken, null, 2),
            JSON.stringify(refreshToken, null, 2),
            JSON.stringify(results, null, 2)
          )
          // If the verifier was wrong, the IdP typically responds with invalid_grant (400)
          if (err && [400,401,403].includes(err.statusCode ?? 0)) {
            console.log(`AudkenniStrategy: invalid_grant trying again`);
            return attempt(); // try next candidate
          }

          // On success, clear stored verifiers so we don't leak them
          if (!err) {
            console.log(`AudkenniStrategy: no error clearing verifier from Redis`);
            self.clearVerifierFromRedis(self._req, `pkceSet`).catch(() => {});
          }
      ...
      console.log(`AudkenniStrategy: attempt called`);

      attempt();
    };

    console.log(`AudkenniStrategy: super.authenticate called`);

    super.authenticate(req, options);
  }

  authorizationParams(options, extra) {
    const params = {};
    console.log(
    ...
    console.log(
      `AudkenniStrategy 1c: extra ${JSON.stringify(extra, null, 2)}`
    );
    ...
    if (this._pkceMethod) {
      ...
      this.storeVerifierInRedis(this._req, `pkceSet`, verifier)
        .then(() =>
          console.log(
  ...
  async storeVerifierInRedis(req, key, value) {
    ...
    try {
      await req.redisClient.sAdd(redisKey, value);
      await req.redisClient.expire(redisKey, 600); // 10-minute TTL
    } catch (err) {
      console.error(
    ...
    console.log(
      `AudkenniStrategy: Stored verifier in Redis set with key: ${redisKey}`
    );
  }
  ...
  async getVerifierFromRedis(req, key) {
    const redisKey = this.generateRedisKey(req, key);
    try {
      const values = await req.redisClient.sMembers(redisKey);
      console.log(
        `AudkenniStrategy: Retrieved ${values.length} verifier(s) from Redis set with key: ${redisKey}`
      );
      return values;
    } catch (err) {
      console.error(
...
module.exports = function () {
  ...
  PassportSSO.logout = function (strategy, options, req, res, cb) {
    ...
    if (redirectUri) {
      ...
      url += `${sep}post_logout_redirect_uri=${encodeURIComponent(redirectUri)}`;
    }
